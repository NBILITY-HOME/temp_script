...
ask_for_api_keys() {
    local env_file="./bolt.diy/.env"
    local example_file=""
    if [ -f "./bolt.diy/.env.production" ]; then
        example_file="./bolt.diy/.env.production"
    elif [ -f "./bolt.diy/.env.example" ]; then
        example_file="./bolt.diy/.env.example"
    else
        echo "Erreur : Aucun fichier modèle d'environnement trouvé."
        exit 1
    fi

    echo ""
    section "🔑 Configuration des clés API pour Bolt.DIY"
    echo "Pour chaque service IA, indique la clé API si tu la possèdes (sinon, laisse vide et tu pourras configurer plus tard)."
    echo ""

    # Récupère toutes les variables *_API_KEY dans un tableau
    mapfile -t api_keys < <(grep -oE "^[A-Z0-9_]+_API_KEY=" "$example_file" | cut -d= -f1)

    for varname in "${api_keys[@]}"; do
        current=$(grep "^$varname=" "$env_file" | cut -d= -f2-)
        if [ -z "$current" ]; then
            echo -n "Entrer la clé pour $varname : "
        else
            echo -n "Entrer la clé pour $varname [$current]: "
        fi
        read -r apikey
        [ -z "$apikey" ] && apikey="$current"
        if grep -q "^$varname=" "$env_file"; then
            sed -i "s|^$varname=.*|$varname=$apikey|" "$env_file"
        else
            echo "$varname=$apikey" >> "$env_file"
        fi
    done
}
...
prepare_bolt_diy() {
    ...
    # APPEL DE LA FONCTION POUR LES QUESTIONS DES APIKEY DES IA
    ask_for_api_keys
