...
check_location_and_clone() {
    ...
    # Vérifier si on est déjà dans le dépôt cloné
    ...
    # On n'est pas dans le dépôt, il faut cloner
    warn "Script exécuté hors du dépôt"
    info "Le dépôt doit être cloné dans : $TARGET_DIR"
    ...
    if [ -d "$TARGET_DIR" ]; then
        ...
        if confirm "Supprimer et re-cloner ?" "N"; then
            ...
            # Relancer (avec ou sans sudo selon contexte)
            if [ "$EUID" -eq 0 ]; then
    ...
    # Authentification GitHub (dépôt privé) - UNE SEULE FOIS
    ...
    # Utiliser ${VAR:-} pour éviter l'erreur "unbound variable"
    ...
    # Créer le dossier logs dans le dépôt fraîchement cloné
    info "Création du dossier logs..."
    ...
    # Relancer le script depuis le dépôt avec les variables GitHub préservées
    ...
    # Relancer le script (avec ou sans sudo selon contexte)
    if [ "$EUID" -eq 0 ]; then
        # Déjà root, relancer directement en préservant les variables
        exec env GH_USER="$GH_USER" GH_TOKEN="$GH_TOKEN" bash "$TARGET_DIR/install_bolt_v10.5.sh"
    else
        # Pas root, relancer avec sudo en préservant les variables
        exec sudo -E bash "$TARGET_DIR/install_bolt_v10.5.sh"
    fi
}
